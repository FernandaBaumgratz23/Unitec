...
import { login, removerLocal, setEmailUsuario, setIdUsuario, setNomeUsuario } from '../../services/auth';
...
export default function LoginAdm() {
    ...
    const onFinish = values => {
        ...
        api.post('Login/EfetuarLoginErp', values).then(res => {
            if (res.status === 200) {
                const token = res.data;
                let dadosUsuario = parseToken(res.data);
                /*setNomeUsuario(dadosUsuario.unique_name);
                setIdUsuario(dadosUsuario.sequenciaUsuario);
